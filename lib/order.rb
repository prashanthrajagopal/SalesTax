...
class Order
  ...
  def calculate_tax
    ...
    @products.each do |product|
      ...
      product_tax << (round_nearest_5_cents(product.price.to_f * tax_rate.to_f)).round(2)
    end
    product_tax
  end
  ...
  def item_price_with_tax
    ...
    @products.each_with_index do |product, index|
      item_price << (product.price.to_f + calculate_tax[index].to_f).round(2)
    end
    item_price
  end
  ...
  def order_receipt
    @products.each_with_index do |product, index|
      puts "#{product.name}\t=>\t#{item_price_with_tax[index]}"
    end
